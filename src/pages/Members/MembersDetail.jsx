...
import { useAuth } from '../../context/AuthContext';
import {
    Users, Briefcase, Building, Target, User, Languages, 
    Smartphone, Mail, Calendar, MapPin, ExternalLink,
    GraduationCap, DollarSign, Globe, Award as CertificateIcon, 
    BookOpen, FileText, Share2, CheckCircle, Star, Phone
} from 'lucide-react';
import AddMember from '../../components/Models/AddMember';
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.headerCard}>
                <div className={styles.profileSection}>
                    <div className={styles.avatarWrapper}>
                        ...
                        {member.symMemberStatus === 'Active' && <div className={styles.statusBadge}></div>}
                    </div>

                    <div className={styles.mainInfo}>
                        <div className={styles.nameRow}>
                            <h1>{member.name}</h1>
                            <span className={`${styles.badge} ${styles.typeBadge}`}>{member.memberType}</span>
                            {member.symMemberStatus === 'Active' && (
                        ...
                        <p className={styles.designation}>{member.designation || member.profession || "Professional"}</p>
                        
                        <div className={styles.quickStats}>
                            <div className={styles.statItem}><MapPin size={16} /> {member.district || "Location N/A"}</div>
                            <div className={styles.statItem}><Calendar size={16} /> Member since {new Date(member.createdAt).getFullYear()}</div>
                            <div className={styles.statItem}><Star size={16} /> {member.workExp || 0} Years Experience</div>
                        </div>
                    </div>
                </div>
                ...
                <div className={styles.actionButtons}>
                    {(user?.role === 'Admin' || String(user?.memberId) === String(member._id) || id === 'me') && (
                        <button className={styles.editBtn} onClick={() => { setEditingMember(member); setShowModal(true); }}>
                ...
                <div className={styles.mainColumn}>
                    <section className={styles.card}>
                        ...
                        <div className={styles.detailsGrid}>
                            <div className={styles.item}>
                                <label>Desired Role</label>
                                <div className={styles.value}>{member.careerProfile?.role || member.preferredJobRole_Sector || "Not specified"}</div>
                            </div>
                            <div className={styles.item}>
                                <label>Industry</label>
                                <div className={styles.value}>{member.careerProfile?.industry || "Not specified"}</div>
                            </div>
                            <div className={styles.item}>
                                <label>Location Preference</label>
                                <div className={styles.value}>{member.preferredJobLocation || "Flexible"}</div>
                            </div>
                            <div className={styles.item}>
                                <label>Expected Salary</label>
                                <div className={styles.value}>{member.careerProfile?.expectedSalary || "Negotiable"}</div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className={styles.card}>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
                            <div style={{paddingLeft: '5px'}}>
                                <h4 style={{margin: '0 0 5px 0', fontSize: '18px', color: '#1e293b'}}>{member.highest_education}</h4>
                                <p style={{color: '#64748b', margin: '0 0 10px 0', display: 'flex', alignItems: 'center', gap: '6px'}}>
                                    <Building size={16} /> {member.educationInstitution || "Institution details not provided"}
                                </p>
                                <div className={`${styles.badge} ${styles.typeBadge}`} style={{display: 'inline-block'}}>
                ...
                <div className={styles.sideColumn}>
                    <section className={styles.card}>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', gap: '20px'}}>
                            <div style={{display: 'flex', gap: '15px'}}>
                                ...
                                <div>
                                    <label style={{fontSize: '12px', color: '#94a3b8', fontWeight: 700}}>MOBILE</label>
                                    <div style={{fontWeight: 600, color: '#334155'}}>{member.mobileNumber || "N/A"}</div>
                                </div>
                            </div>
                            <div style={{display: 'flex', gap: '15px'}}>
                                <Mail size={20} color="#64748b" />
                                <div>
                                    <label style={{fontSize: '12px', color: '#94a3b8', fontWeight: 700}}>EMAIL</label>
                                    <div style={{fontWeight: 600, color: '#334155', wordBreak: 'break-all'}}>{member.email || "N/A"}</div>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className={styles.card}>
                        ...
                        <div style={{display: 'flex', flexWrap: 'wrap', gap: '10px'}}>
                            {member.languages?.length > 0 ? member.languages.map(lang => (
                                <span key={lang} style={{background: '#f1f5f9', color: '#475569', padding: '8px 16px', borderRadius: '10px', fontSize: '14px', fontWeight: 600}}>
                                    {lang}
                                </span>
                            )) : <p style={{color: '#94a3b8', fontSize: '14px'}}>No languages listed</p>}
                        </div>
                    </section>

                    {/* Resume Card */}
                    {(member.resume || member.resumeLink) && (
                        <section className={styles.card} style={{background: '#0084ff', border: 'none'}}>
                            ...
                            <a 
                                ...
                                target="_blank" 
                                rel="noreferrer"
                                style={{display: 'block', textAlign: 'center', background: 'white', color: '#0084ff', padding: '12px', borderRadius: '12px', fontWeight: 700, textDecoration: 'none'}}
                            >
                                View CV
                            </a>
                        </section>
            ...
            <AddMember
                ...
                editMember={editingMember}
                onSuccess={(updated) => { setMember(updated); setShowModal(false); }}
            />
        </div>
    );
}
