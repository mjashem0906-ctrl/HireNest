...
// --- INTERNAL COMPONENT: ProvidedForm ---
const ProvidedForm = ({ isOpen, onClose, onSubmit, initialData }) => {
    // --- 1. MERGED STATE (Includes all fields from both files) ---
    const [title, setTitle] = useState('');
    const [companyName, setCompanyName] = useState('');
    const [employmentType, setEmploymentType] = useState(EMPLOYMENT_TYPES[0]);
    const [location, setLocation] = useState('');
    ...
    // Inline styles from the second file (preserves the look)
    const modalStyles = {
        overlay: {
        ...
        modal: {
            ...
            width: '600px', maxWidth: '90%', position: 'relative',
        ...
        label: {
            display: 'block', marginBottom: '5px', fontWeight: '500', fontSize: '14px'
        },
        row: {
...
function Jobs() {
    ...
    // 👇 UPDATED HELPER: Maps DB status to Pipeline status including Review and Offer
                                                                    ...
                                                                    {request.appliedMembers.map((app, idx) => (
                                                                        <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
                                                                            ...
                                                                            <td style={{ padding: '10px' }}>
                                                                                {/* UPDATED DROPDOWN WITH REVIEW AND OFFER */}
