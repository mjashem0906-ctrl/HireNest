...
//   if (!referee) return <div className={styles.loading}><div className={styles.spinner}></div></div>;

//   return (
//     <div className={styles.pageContainer}>
//       <div className={styles.banner}>
//         <div className={styles.topNav}>
//           <button onClick={() => navigate('/referees')} className={styles.backButton}>
//             <ArrowLeft size={18} /> Back to Directory
//           </button>
//           <div className={styles.headerActions}>
//             {isEditing ? (
//               <>
//                 <button onClick={() => setIsEditing(false)} className={`${styles.actionBtn} styles.cancel`}>
//                   <X size={16} /> Cancel
//                 </button>
//                 <button onClick={handleSave} className={`${styles.actionBtn} ${styles.save}`} disabled={loading}>
//                   {loading ? <div className={styles.miniSpinner}></div> : <Save size={16} />} Save Changes
//                 </button>
//               </>
//             ) : (
//               <button onClick={() => setIsEditing(true)} className={`${styles.actionBtn} ${styles.edit}`}>
...
//               style={{
//                 backgroundColor: formData.solidarityMemberStatus === 'Active' ? '#dcfce7' : '#f1f5f9',
//                 color: formData.solidarityMemberStatus === 'Active' ? '#166534' : '#64748b'
//               }}
//             >
//               {formData.solidarityMemberStatus?.toUpperCase()}
//             </div>
//           </div>
...
//               <span><MapPin size={16} /> {formData.district || 'Location'}</span>
//               <span><Star size={16} /> {formData.referrerStatus || 'Verified'}</span>
//               <span><Briefcase size={16} /> {formData.referringSector || 'General'}</span>
//             </div>
//           </div>
//         </div>
//       </div>
...
//           <section className={styles.infoCard} style={{animationDelay: '0.1s'}}>
//             <div className={styles.cardHeader}><User size={20}/> Personal Details</div>
//             <div className={styles.detailsList}>
//               {['name', 'gender', 'email', 'mobileNumber', 'age'].map(field => (
...
//           <section className={styles.infoCard} style={{animationDelay: '0.2s'}}>
//             <div className={styles.cardHeader}><Building2 size={20}/> Professional Context</div>
//             <div className={styles.detailsList}>
//               {['occupation', 'companyDetails', 'sector', 'referringSector'].map(field => (
//                 <div key={field} className={styles.item}>
//                   <label>{field.replace(/([A-Z])/g, ' $1')}</label>
//                   {isEditing ? (
//                     <input 
...
//           <section className={styles.infoCard} style={{animationDelay: '0.3s'}}>
//             <div className={styles.cardHeader}><FileText size={20}/> Additional Context</div>
//             <div className={styles.detailsList} style={{gridTemplateColumns: '1fr'}}>
...
//                   />
//                 ) : (
//                   <div className={styles.value} style={{lineHeight: 1.6}}>{formData.opportunityDescription || 'No description provided.'}</div>
//                 )}
//               </div>
//             </div>
//           </section>
...
//           <section className={styles.infoCard} style={{animationDelay: '0.4s'}}>
//             <div className={styles.cardHeader}><Target size={20}/> Referring Scope</div>
//             <div className={styles.detailsList} style={{gridTemplateColumns: '1fr'}}>
//               {[
//                 {label: 'Referring For', field: 'referringFor', icon: <Users size={16}/>},
//                 {label: 'Job Offer Type', field: 'jobOfferType', icon: <Layers size={16}/>},
//                 {label: 'Level of Support', field: 'levelOfSupport', icon: <Award size={16}/>}
//               ].map(item => (
//                 <div key={item.field} className={styles.item}>
//                   <label style={{display:'flex', gap:6, alignItems:'center'}}>{item.icon} {item.label}</label>
//                   {isEditing ? (
//                     <input 
//                       value={formData[item.field]} 
//                       onChange={(e) => handleInputChange(item.field, e.target.value)}
...
//                     />
//                   ) : (
//                     <div className={styles.value}>{formData[item.field] || '—'}</div>
//                   )}
//                 </div>
//               ))}
//             </div>
...
//           <section className={styles.infoCard} style={{animationDelay: '0.5s'}}>
//             <div className={styles.cardHeader}><MapPin size={20}/> Location Data</div>
//             <div className={styles.detailsList} style={{gridTemplateColumns: '1fr'}}>
//               {['district', 'address', 'offer_Location'].map(field => (
//                 <div key={field} className={styles.item}>
...
const RefereeDetailsPage = () => {
  ...
  return (
    <div className={styles.pageContainer}>
      ...
      <div className={styles.profileHeaderCard}>
        <div className={styles.heroContent} style={{display:'flex', gap: '30px', alignItems:'center'}}>
      ...
      <div className={styles.contentGrid}>
          ...
          <section className={styles.infoCard}>
            ...
            <div className={styles.detailsList}>
              <div className={styles.item}>
                ...
                {isEditing ? <input className={styles.editInput} value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}/> : <div className={styles.value}>{formData.name}</div>}
              </div>
              <div className={styles.item}>
                <label>Email Address</label>
                {isEditing ? <input className={styles.editInput} value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})}/> : <div className={styles.value}>{formData.email}</div>}
              </div>
              <div className={styles.item}>
                <label>Mobile Number</label>
                {isEditing ? <input className={styles.editInput} value={formData.mobileNumber} onChange={e => setFormData({...formData, mobileNumber: e.target.value})}/> : <div className={styles.value}>{formData.mobileNumber}</div>}
              </div>
            </div>
          </section>
          ...
          <section className={styles.infoCard}>
            ...
            <div className={styles.detailsList}>
              <div className={styles.item}>
                ...
                {isEditing ? <input className={styles.editInput} value={formData.occupation} onChange={e => setFormData({...formData, occupation: e.target.value})}/> : <div className={styles.value}>{formData.occupation}</div>}
              </div>
              <div className={styles.item}>
                <label>Company</label>
                {isEditing ? <input className={styles.editInput} value={formData.companyDetails} onChange={e => setFormData({...formData, companyDetails: e.target.value})}/> : <div className={styles.value}>{formData.companyDetails}</div>}
              </div>
            </div>
          </section>
        </div>
        ...
        <div className={styles.rightCol}>
           <section className={styles.infoCard}>
            <div className={styles.cardHeader}><Target size={20}/> Support Info</div>
            <div className={styles.detailsList} style={{gridTemplateColumns: '1fr'}}>
              <div className={styles.item}>
                <label>Referring For</label>
                {isEditing ? <input className={styles.editInput} value={formData.referringFor} onChange={e => setFormData({...formData, referringFor: e.target.value})}/> : <div className={styles.value}>{formData.referringFor || '—'}</div>}
              </div>
              <div className={styles.item}>
                <label>Offer Location</label>
                {isEditing ? <input className={styles.editInput} value={formData.offer_Location} onChange={e => setFormData({...formData, offer_Location: e.target.value})}/> : <div className={styles.value}>{formData.offer_Location || '—'}</div>}
              </div>
            </div>
          </section>
        </div>
