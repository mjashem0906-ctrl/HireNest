      ...
      {user?.profileCompleted < 100 && (
        <div className={styles.profileCompletionSection}>
          ...
          <div className={styles.progressBar}>
            <div className={styles.progressFill} style={{ width: `${user?.profileCompleted}%` }}></div>
          </div>
      ...
      <div className={styles.mainGrid}>
        ...
        <section className={styles.jobSection}>
          ...
          <div className={styles.jobsList}>
            {data.recentJobs.length > 0 ? data.recentJobs.map(job => {
              const initials = (job.title?.split(' ').slice(0, 2).map(w => w[0]).join('') || 'J').toUpperCase();
              return (
                <div key={job._id} className={styles.jobCardLarge} onClick={() => navigate(`/jobs/${job._id}`)}>
                  <div className={styles.jobCardHeader}>
                    <div className={styles.jobCompanyIcon}>{initials}</div>
                    <div className={styles.jobHeaderInfo}>
                      <h4>{job.title}</h4>
                      <p><Building size={14} /> {job.companyName}</p>
                    </div>
                  ...
                  <div className={styles.jobCardMeta}>
                    ...
                    <span className={styles.salaryRange}>{job.salary || '₹10-15 LPA'}</span>
                  </div>
        ...
        <section className={styles.mentorSection}>
          ...
          <div className={styles.mentorsList}>
            {data.newMentors.length > 0 ? data.newMentors.map(mentor => (
              <div key={mentor._id} className={styles.mentorCardLarge} onClick={() => navigate(`/member/${mentor._id}`)}>
