...
const MentorsPage = () => {
  ...
  const handleConnectClick = (e, mentorObj) => {
    if (e && typeof e.stopPropagation === "function") {
      e.stopPropagation();
    }

    const targetMentor = mentorObj || (e && e._id ? e : null);
  ...
  const availableConnectDomains = React.useMemo(() => {
    if (!selectedMentor) return allDomains;
    const set = new Set();
    const add = (val) => {
      if (!val) return;
    ...
    add(selectedMentor.fieldofStudy_Interest);
    add(selectedMentor.branch);
    add(selectedMentor.domain);
    add(selectedMentor.domains);
    add(selectedMentor.careerProfile?.industry);
    add(selectedMentor.careerProfile?.department);
    add(selectedMentor.offeringSector);
    add(selectedMentor.preferredJobRole_Sector);

    const list = Array.from(set).sort();
    return list.length > 0 ? list : (allDomains.length > 0 ? allDomains : ["General"]);
  }, [selectedMentor, allDomains]);

  const availableConnectSkills = React.useMemo(() => {
    if (!selectedMentor) return allSkills;
    const set = new Set();
    const add = (val) => {
      if (!val) return;
      if (Array.isArray(val)) {
    ...
    add(selectedMentor.skills);
    add(selectedMentor.skillsToImprove);

    const list = Array.from(set).sort();
    return list.length > 0 ? list : (allSkills.length > 0 ? allSkills : ["Mentorship", "Career Guidance"]);
  }, [selectedMentor, allSkills]);
